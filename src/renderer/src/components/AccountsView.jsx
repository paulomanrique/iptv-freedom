...
function Detail({ account, onRemove, onSetActive, onEdit, isActive }) {
    ...
    <div className="p-5">
      ...
      <div className="flex flex-col gap-2 mt-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => onEdit(account)} className="flex-1 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-2xs font-medium text-white/80">
            {t('accounts.edit')}
          </button>
          <button onClick={() => onRemove(account.id)} className="flex-1 py-2 rounded-lg bg-white/10 hover:bg-red-500/30 text-2xs font-medium text-white/80">
            {t('accounts.remove')}
          </button>
        </div>
      </div>
...
export default function AccountsView({ accounts, activeId, selectedId, onSelect, onAdd, onRemove, onSetActive, onEdit }) {
  const { t } = useTranslation()
