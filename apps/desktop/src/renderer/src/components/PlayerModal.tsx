    ...
    <div className="fixed inset-0 z-50 grid place-items-center p-4 sm:p-8">
      ...
      >
        <div className="relative aspect-video bg-black">
          ...
          <div
            className={`absolute top-0 inset-x-0 px-4 py-3 flex items-center gap-3 bg-gradient-to-b from-black/75 via-black/30 to-transparent transition-opacity duration-300 ${chrome ? "opacity-100" : "opacity-0"}`}
          >
            {isLive && (
            ...
            {isLive && stats && (
              <span className="flex items-center gap-2 text-[11px] tabular-nums text-white/90 bg-black/40 backdrop-blur rounded-full px-2.5 py-1 shrink-0">
                ...
                {stats.dropped > 0 && (
                  <>
                    <span className="text-white/30">·</span>
                    <span className="text-white/60" title={t("player.dropsTip")}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[15px] font-semibold text-white truncate drop-shadow">
                {item.name}
              </div>
            </div>
            {isLive && (
