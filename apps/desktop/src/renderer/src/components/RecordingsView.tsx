...
import { formatBytes, formatSpeed, formatDuration, recordingLabel } from "../format";
...
function Row({ r, seed, now, onStop, onOpen, onRemove }: RowProps) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5">
      <Poster icon={r.icon} seed={seed} className="h-10 w-7" />
      <div className="flex-1 min-w-0">
        <div className="flex justify-between gap-2">
          ...
          <span
            className={`text-[10px] shrink-0 ${active ? "text-destructive" : err ? "text-destructive" : "text-muted-foreground"}`}
          >
