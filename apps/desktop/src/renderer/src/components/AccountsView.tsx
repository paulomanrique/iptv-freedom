...
function Detail({ account, onRemove, onSetActive, onEdit, isActive }: DetailProps) {
  ...
  const rows = ui
    ? [
        ...
        [t('accounts.format'), (Array.isArray(ui.allowed_output_formats) ? ui.allowed_output_formats : []).join(', ').toUpperCase() || '—'],
    ...
    <div className="p-5">
      ...
      {state === 'ok' && (
        <>
          ...
          {days != null && (
            <div className="mt-4">
              <div className="flex justify-between text-[10px] text-muted-foreground mb-1"><span>{t('accounts.daysLeft')}</span><span>{days}</span></div>
      ...
      <div className="flex flex-col gap-2 mt-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => onEdit(account)} className="flex-1 py-2 rounded-lg bg-muted hover:bg-accent text-xs font-medium text-foreground">
            {t('accounts.edit')}
          </button>
          <button onClick={() => onRemove(account.id)} className="flex-1 py-2 rounded-lg bg-muted hover:bg-destructive/30 text-xs font-medium text-foreground">
            {t('accounts.remove')}
          </button>
        </div>
      </div>
...
export default function AccountsView({ accounts, activeId, selectedId, onSelect, onAdd, onRemove, onSetActive, onEdit }: AccountsViewProps) {
  const { t } = useTranslation()
    ...
    <>
      <section className="flex-1 min-w-0 scroll overflow-y-auto">
        <div className="p-4 space-y-2">
          {accounts.length === 0 && (
            <div className="text-xs text-muted-foreground text-center py-10">{t('accounts.empty')}</div>
          )}
          {accounts.map((a) => (
            <div
              key={a.id}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold truncate flex items-center gap-2">
                  {a.name || a.host}
                  {a.id === activeId && <span className="text-[9px] bg-success/20 text-success rounded px-1.5 py-0.5">{t('accounts.active')}</span>}
                </div>
      ...
      <aside className="w-80 shrink-0  border-s border-border scroll overflow-y-auto">
        {selected ? (
          <Detail account={selected} onRemove={onRemove} onSetActive={onSetActive} onEdit={onEdit} isActive={selected.id === activeId} />
        ) : (
