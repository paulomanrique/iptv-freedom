...
export default function FavoritesView({ account, favorites, onPlay, onDownload, fav }: FavoritesViewProps) {
  ...
  return (
    <>
      ...
      <div className="w-56 shrink-0  border-e border-border flex flex-col">
        <div className="px-3 py-2 text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">{t('nav.favorites')}</div>
            ...
            <div className="divide-y divide-border">
              {items.map((m) => (
                <div key={`${m.kind}:${m.id}`} className={`flex items-center gap-3 px-4 py-2 cursor-pointer ${String(m.id) === String(selected?.id) ? 'bg-accent' : 'hover:bg-accent'}`} onClick={() => setSelectedId(m.id)} onDoubleClick={() => playItem(m)}>
                  ...
                  {m.kind === 'live' && <span className="text-[10px] text-destructive flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-destructive" />{t('common.liveBadge')}</span>}
                </div>
              ))}
            </div>
          )}
