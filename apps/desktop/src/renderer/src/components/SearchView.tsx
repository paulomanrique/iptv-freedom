    ...
    return (
      <section className="flex-1 grid place-items-center text-xs text-muted-foreground text-center px-8">
        <Trans
          ...
          values={{ query }}
          components={{ b: <b className="text-muted-foreground mx-1" /> }}
        />
      </section>
    );
  }
    ...
    <>
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] text-muted-foreground">
                  {g.total > g.items.length ? `${g.items.length}+` : g.items.length}
                </span>
              </button>
            );
          })}
          <div className="px-3 pt-2 text-[10px] text-muted-foreground">
            {t("search.total", { count: totalResults })}
          </div>
        </div>
      </div>
      ...
      <section className="flex-1 min-w-0 flex flex-col">
        <div className="px-4 py-2 text-xs text-muted-foreground border-b border-border truncate shrink-0">
          {activeGroup ? labelFor(activeGroup.kind) : ""}
