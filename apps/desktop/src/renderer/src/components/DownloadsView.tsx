    ...
    <div className="flex items-center gap-3 px-4 py-2.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between gap-2">
          ...
          <span className={`text-[10px] shrink-0 ${done ? 'text-success' : err ? 'text-destructive' : 'text-muted-foreground'}`}>
      ...
      <div className="flex items-center gap-1 shrink-0">
        {done && (
          <button title={t('downloads.openFolder')} onClick={() => onOpen(d.id)} className="h-7 w-7 grid place-items-center rounded-md hover:bg-accent text-muted-foreground">
        ...
        {active && (
          <button title={t('downloads.pause')} onClick={() => onPause(d.id)} className="h-7 w-7 grid place-items-center rounded-md hover:bg-accent text-muted-foreground">
            <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M6 5h4v14H6zM14 5h4v14h-4z" /></svg>
          </button>
        )}
        {(d.status === 'paused' || err) && (
          <button title={t('downloads.resume')} onClick={() => onResume(d.id)} className="h-7 w-7 grid place-items-center rounded-md hover:bg-accent text-muted-foreground">
            <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
          </button>
        )}
        {!done && (
          <button title={t('downloads.cancel')} onClick={() => onCancel(d.id)} className="h-7 w-7 grid place-items-center rounded-md hover:bg-destructive/30 text-muted-foreground">
...
export default function DownloadsView({ downloads, onPause, onResume, onCancel, onOpen, onClearCompleted }: DownloadsViewProps) {
  const { t } = useTranslation()
  ...
  return (
    <section className="flex-1 min-w-0 flex flex-col">
      <div className="px-4 py-2 border-b border-border flex items-center justify-between shrink-0">
        <span className="text-xs text-muted-foreground">{t('downloads.itemCount', { count: downloads.length })}</span>
        <button
        ...
        >
          ...
          {t('downloads.clearCompleted')}{completedCount > 0 ? ` (${completedCount})` : ''}
        </button>
      </div>

      <div className="flex-1 scroll overflow-y-auto">
        {downloads.length === 0 ? (
          <div className="p-10 text-xs text-muted-foreground text-center">{t('downloads.empty')}</div>
        ) : (
