...
export default function LibraryView({ account, kind, viewStyle, onPlay, onDownload, fav }: LibraryViewProps) {
  ...
  return (
    <>
      ...
      <div className="w-56 shrink-0  border-e border-border flex flex-col">
        <div className="px-3 py-2 text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">{t('library.categories')}</div>
        <div className="flex-1 scroll overflow-y-auto py-1">
          {catLoading && <div className="flex items-center gap-2 text-xs text-muted-foreground px-3 py-2"><span className="h-3 w-3 rounded-full border-2 border-muted-foreground/30 border-t-foreground animate-spin" />{t('library.loading')}</div>}
          {categories.map((c) => {
            ...
            return (
              ...
              >
                ...
                <svg className="h-3 w-3 text-muted-foreground shrink-0 rtl:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m9 6 6 6-6 6" /></svg>
              </button>
            )
          })}
        </div>
      ...
      <section className="flex-1 min-w-0 flex flex-col">
        <div className="px-4 py-2 text-xs text-muted-foreground border-b border-border truncate shrink-0">
          ...
          {!loading && <span className="text-muted-foreground"> · {visible.length}{visible.length === MAX_RESULTS ? '+' : ''}</span>}
        </div>

        <div className="flex-1 scroll overflow-y-auto">
          {error && <div className="m-4 text-xs text-destructive bg-destructive/10 rounded-lg px-3 py-2">{t('library.error', { error })}</div>}
          {loading && <div className="flex items-center gap-2 text-xs text-muted-foreground p-4"><span className="h-3.5 w-3.5 rounded-full border-2 border-muted-foreground/30 border-t-foreground animate-spin" />{t('library.loading')}</div>}
          {!loading && visible.length === 0 && !error && <div className="p-6 text-xs text-muted-foreground text-center">{t('library.empty')}</div>}

          {!loading && visible.length > 0 && (useGrid ? (
            <div className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-3 p-4">
              {visible.map((m) => (
                <div key={m.id} className={`cursor-pointer rounded-lg p-1 ${m.id === selected?.id ? 'bg-accent' : 'hover:bg-accent'}`} onClick={() => setSelectedId(m.id)} onDoubleClick={() => playItem(m)}>
            ...
            <div className="divide-y divide-border">
              {visible.map((m) => (
                <div key={m.id} className={`flex items-center gap-3 px-4 py-2 cursor-pointer ${m.id === selected?.id ? 'bg-accent' : 'hover:bg-accent'}`} onClick={() => setSelectedId(m.id)} onDoubleClick={() => playItem(m)}>
                  ...
                  {kind === 'live' && <span className="text-[10px] text-destructive flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-destructive" />{t('common.liveBadge')}</span>}
                  {kind === 'vod' && m.ext && <span className="text-[10px] text-muted-foreground uppercase">{m.ext}</span>}
                </div>
              ))}
            </div>
          ))}
          {visible.length === MAX_RESULTS && <div className="px-4 py-2 text-[10px] text-muted-foreground">{t('library.showingFirst', { max: MAX_RESULTS })}</div>}
        </div>
      </section>
